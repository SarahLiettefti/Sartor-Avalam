...
def casejouables(body):
    game=body["game"]
    casesoccupes = []#toutes cases "jouable" #au début 48
    li = 0
    for game[li] in game:
        col = 0
...
def wichplayer(body):#renvoie la couleur joué
...
def findallcoup(game,de,to,coups,ecrase,ecrase2,ecrase3,ecrase4,ajout,embete,player,completusbyus,complettheybyus,complettheybythey):
    move = [de,to]
    coups.append(move)
    if game[de[0]][de[1]][-1]==player:#si le pion qu'on va déplacer est a nous
        if game[to[0]][to[1]][-1]==player:#si on le déplace sur un de nos pion
            ajout.append(move)
            if ((len(game[de[0]][de[1]]))+(len(game[to[0]][to[1]])))==5: #si ça complète une tour de 5 pion
...
def coup(body):
    ...
    ecrase2=[] #mets un de nos pions sur l'autre et creer un tour de 2
    ecrase3=[] #mets un de nos pions sur l'autre et creer un tour de 3
    ecrase4=[] #mets un de nos pions sur l'autre et creer un tour de 4
    ajout=[]#met un de nos pions sur un notre a nous
    embete=[]#met un des pions adversaire sur un autre a lui
    completusbyus = [] #complet un tour de 5 par nos couleur
    complettheybyus = [] #complet un tour de 5 par les couleur ennemie (apres l'avoir completé ce sont des pions ennemis qui ne savent plus bouger)
    complettheybythey = []
    seul=[]#liste de déplacement dont l'origine ne peut faire que ce déplacement
    seulvoisin=[] #une liste des cases qui n'ont qu'un déplacement possible
    ensembleseul=[]#déplacement d'une case qui ne peut que etre deplacée qu'a un endroit et qui écrase l'adversaire sur une tour qui ne pouvait aussi etre déplacer que à cet endroit
    for line in range(9):
        for col in range(9):
            de=[line,col]
